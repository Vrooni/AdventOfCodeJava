...
public class Day16_2 {
    ...
    public String part2(List<String> input) {
        ...
        return String.valueOf(sitPositions.size());
    }

    private void addSitPositions(Node end, Position start, Map<Node, Integer> distances, Set<Position> sitPositions, char[][] map) {
    ...
    private Map<Node, Integer> getDistances(Position start, char[][] map) {
    ...
    private List<Node> getNeighboursReversed(Node current, char[][] map) {
        List<Node> neighbours = new ArrayList<>();
    ...
    private Position nextPositionReversed(Node current) {
        return switch (current.direction) {
            case LEFT -> new Position(current.x+1, current.y);
            case RIGHT -> new Position(current.x-1, current.y);
            case UP -> new Position(current.x, current.y+1);
