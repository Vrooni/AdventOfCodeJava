...
public class Day21_2 {
    ...
    public String part2(List<String> input) {
        List<Rule> rules = input.stream().map(this::getRule).toList();
        Grid grid = getGrid(".#./..#/###");
        ...
        return String.valueOf(grid.grid.stream()
                .mapToLong(girdLine -> girdLine.stream()
                        .filter(pixel -> pixel)
                        .count()).sum());
    }

    private Rule getRule(String rule) {
    ...
    private Grid getGrid(String gridString) {
    ...
    private List<Grid> getVariations(Grid grid) {
    ...
    private List<List<Grid>> split(Grid grid, int n) {
        //n = 2
    ...
    private Grid concat(List<List<Grid>> squares) {
        int m = squares.size();
        int n = squares.getFirst().getFirst().grid.size();
    ...
    private Grid getOutput(Grid input, List<Rule> rules) {
        for (Rule rule : rules) {
