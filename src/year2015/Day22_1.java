...
public class Day22_1 {
    ...
    public String part1(List<String> lines) {
        //IMPORTANT!!! Run 3 times to insure output is right, because random
        int bossHitPoints = Integer.parseInt(lines.get(0).replace("Hit Points: ", ""));
        int damage = Integer.parseInt(lines.get(1).replace("Damage: ", ""));
        int spentMana = Integer.MAX_VALUE;

        for (int i = 0; i < 100000; i++) {
            spentMana = Math.min(spentMana, doFight(bossHitPoints, damage));
        }

        return String.valueOf(spentMana);
    }

    private int doFight(int bossHP, int damage) {
        int playerHP = 50;
        int mana = 500;
        boolean turn = true;
    ...
    private List<Integer> getPossibleSpells(int shield, int poison, int recharge, int mana) {
