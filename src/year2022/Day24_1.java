...
public class Day24_1 {
    ...
    public String part1(List<String> lines) {
        ...
        final int maxWidth = lines.getFirst().length();
        ...
        Result result = moveThrowMap(map, start, end, maxWidth, maxHeight);
        return String.valueOf(result.time);
    }

    private Position getStartPosition(Map<Position, List<Element>> map) {
        int x = 0;
        while (true) {
    ...
    private Position getEndPosition(Map<Position, List<Element>> map, int y) {
        int x = 0;
        while (true) {
    ...
    private List<Position> possibleDirections(Map<Position, List<Element>> map, Position currentPosition) {
    ...
    private Result moveThrowMap(Map<Position, List<Element>> map, Position start,  Position end, int maxWidth, int maxHeight) {
        ...
        while (!directions.isEmpty()) {
            ...
            map = mapSteps.computeIfAbsent(currentStep.time(),
                    _ -> moveMap(mapSteps.get(currentStep.time() - 1), maxWidth, maxHeight)
            );
