...
public class Day15_1 {
    ...
    private static class Entity implements Comparable<Entity> {
        ...
        public Position getMove(char[][] map) {
            ...
            return moves.getFirst().dist == Integer.MAX_VALUE ? this.position : moves.getFirst().position;
        }
        ...
        public void dealDamage(char[][] map, int attackPower) {
            this.hp -= attackPower;
        ...
        private Entity getEntity(List<Entity> entities, int x, int y) {
            return entities.stream()
                    .filter(entity -> entity.position.x() == x && entity.position.y() == y)
                    .findFirst().get();
        }

        private List<Position> getNeighbours(Position pos, char[][] map) {
            int x = pos.x();
            int y = pos.y();
    ...
    public String part1(List<String> input) {
        ...
        return String.valueOf(
                fight(entities, map) * entities.stream().mapToInt(entity -> entity.hp).sum()
        );
    }

    private int fight(List<Entity> entities, char[][] map) {
        for (int round = 0; true; round++) {
            for (Entity entity : entities) {
                ...
                if (enemy != null) {
                    enemy.dealDamage(map, entity.attackPower);
                }
            }
    ...
    private List<Entity> getEntities(List<String> input) {
    ...
    private boolean hasTargets(List<Entity> entities) {
