...
public class Day16_2 {
    ...
    private Map<Integer, Opcode> getOpcodes(List<Sample> samples) {
        ...
        while (!queue.isEmpty()) {
            ...
            if (possibleOpcodes.size() == 1) {
                taken.add(possibleOpcodes.getFirst());
                opcodes.put(sample.instruction[0], possibleOpcodes.getFirst());
            } else {
                queue.add(sample);
            }
        }
    ...
    private void runTests(Map<Integer, Opcode> opcodes, int[] registers, List<int[]> tests) {
        tests.forEach(test -> {
