...
public class Day24_2 {
    ...
    public String part2(List<String> input)  {
        ...
        for (List<Position> permutation : permutations) {
            Position first = permutation.getFirst();
            Position last = permutation.getLast();
    ...
    private List<Position> getVisitPoints(List<String> input) {
    ...
    private int getMinDistance(Position start, Position end, List<String> input) {
    ...
    private void getPermutations(List<Position> original, int element, List<List<Position>> permutations) {
        for (int i = element; i < original.size(); i++) {
            Collections.swap(original, i, element);
            getPermutations(new ArrayList<>(original), element + 1, permutations);
            Collections.swap(original, element, i);
        }
