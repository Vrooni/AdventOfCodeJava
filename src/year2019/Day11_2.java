...
public class Day11_2 {
    ...
    public String part2(List<String> lines) {
        ...
        Map<Position, Boolean> panels = runProgram(program);
        List<Position> coordinates = panels.keySet().stream().toList();

        int minX = coordinates.stream().mapToInt(Position::x).min().getAsInt();
        int maxX = coordinates.stream().mapToInt(Position::x).max().getAsInt();
        int minY = coordinates.stream().mapToInt(Position::y).min().getAsInt();
        int maxY = coordinates.stream().mapToInt(Position::y).max().getAsInt();

        StringBuilder result = new StringBuilder();
        for (int y = minY; y <= maxY; y++) {
            for (int x = minX; x <= maxX; x++) {
                result.append(panels.getOrDefault(new Position(x, y), false) ? "#" : " ");
            }
            result.append("\n");
        }
